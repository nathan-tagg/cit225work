...
SELECT s1.system_user_id   AS user_id
,      s1.system_user_name AS user_name
,      s2.system_user_id   AS cby_user_id
,      s2.system_user_name AS cby_user_name
,      s3.system_user_id   AS lby_user_id
,      s3.system_user_name AS lby_user_name
FROM system_user s1 
INNER JOIN system_user s2 ON s1.created_by = s2.system_user_id
INNER JOIN system_user s3 ON s2.last_updated_by = s3.system_user_id;
...
ON       e.department_id = d.department_id INNER JOIN salary s
ON       e.salary_id = s.salary_id
WHERE    (TRUNC(SYSDATE) + 1) > TRUNC(s.effective_start_date) AND (TRUNC(SYSDATE) - 60) < TRUNC(s.effective_start_date)
GROUP BY d.department_name
ORDER BY d.department_name;
